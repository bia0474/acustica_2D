...
# ------------------------------------------------------------
# Wiggle plot com normalizacao POR PAINEL 
...
margin = Nboudary # ignora zona de borda/ABC
...
for p, ix in enumerate(x_positions):
    ...
    panel_data = all_bins[:, ix, :].T  # (nz, n_gathers)
    ...
    for b in range(n_gathers):
        trace = all_bins[b, ix, :]
        x_offset = panel_offset + b * trace_spacing + trace_spacing/2
        plot_wiggle_overlay(ax, trace, z_axis, x_offset, panel_max, lw=0.4)
